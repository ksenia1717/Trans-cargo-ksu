...
export default function Calculator2() {
    ...
    // ТО что будет вводить ползватель в инпутах
    const [routeId, setRouteId] = useState(CALCULATOR_ROUTES[0].id)
    const [weight, setWeight] = useState('')
    const [volume, setVolume] = useState('')

    const route = CALCULATOR_ROUTES.find((r) => r.id === routeId)

    // const priceKg =  route.pricePerKg ? Number(route.pricePerKg) : 0
    // const priceM3 = route.pricePerM3 ? Number(route.pricePerM3) : 0

    const weigthInp = parseFloat(String(weight).replace(',', '.') ) || 0
    const volumeInp = parseFloat(String(volume).replace(',', '.')) || 0

    const cost = Math.max((weigthInp * route.pricePerKg), (volumeInp * route.pricePerM3), route.minSum) || 0

    const formatedCost = cost.toLocaleString('ru-RU', {style: 'currency', currency: 'RUB'})
    ...
    <>
      <div className='calculator-page container'>
        ...
        <div className='calculator-forms'>
          ...
          <div>
            <p>Стоимост перевозки: <span className='blue-p'>~{formatedCost} руб</span></p>
